...
def process(input_lvs, output_lv, rule_base, crisp_values):
    ...
    implication_fuzzy_sets = implication(fuzzy_values, rules, output_lv)[0]
    result_fuzzy_set = aggregation(*implication_fuzzy_sets)

    crisp_result = defuzzifier.defuzzification(output_lv['U'], implication_fuzzy_sets, 'cog')
    word_result = []
    for term, mf in output_lv['terms'].items():
        sm = defuzzifier.jaccard_measure(implication_fuzzy_sets, mf)
